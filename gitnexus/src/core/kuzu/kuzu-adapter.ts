...
export const loadExistingGraph = async (
  ...
  try {
    ...
    for (const row of await result.getAll()) {
      ...
      // All relationships are regenerated by the incremental pipeline:
      // CONTAINS (processStructure), IMPORTS/CALLS/EXTENDS/IMPLEMENTS
      // (full import/call/heritage), MEMBER_OF/STEP_IN_PROCESS (communities/processes).
      // Only keep DEFINES for unchanged nodes (File -> code element).
      const type = str(row.type ?? row[2]);
      if (type !== 'DEFINES') continue;
      // For DEFINES: use the canonical ID format to avoid duplicates with freshly parsed results
      const canonicalId = `DEFINES:${fromId}->${toId}`;
      relationships.push({
        id: canonicalId,
        sourceId: fromId,
        targetId: toId,
